...
RSpec.describe StaffMember, type: :model do
  describe "#password=" do
    example "文字列を与えると、hashed_passwordは長さ60の文字列になる" do
      ...
      expect(member.hashed_password).to be_kind_of(String) # ターゲットがあるクラスのインスタンスであるか調べる
  ...
  describe "値の正規化" do
    ...
    example "emailに含まれる全角英数字記号を半角に変換" do
      member = create(:staff_member, email: "ｔｅsｔ@example.com")
      expect(member.email).to eq("test@example.com")
    end
  ...
  describe "バリデーション" do
    example "@を2個含むemailは無効" do
      member = build(:staff_member, email: "test@@example.com") # buildにすることでバリデーションがテストできる
      expect(member).not_to be_valid
    end

    example "アルファベット表記のfamily_nameは有効" do
      member = build(:staff_member, family_name: "Alen")
    ...
    example "漢字を含むfamily_nama_kanaは無効" do
